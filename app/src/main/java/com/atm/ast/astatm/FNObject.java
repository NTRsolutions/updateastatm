...
import com.atm.ast.astatm.helper.IObjectHelper;
import com.atm.ast.astatm.utils.FNObjectUtil;
import com.atm.ast.astatm.utils.FNSymbols;
...
public class FNObject implements Cloneable, Parcelable, IObjectHelper {
	...
	public static final Creator<FNObject> CREATOR = new Creator<FNObject>() {
		@Override
		public FNObject createFromParcel(Parcel in) {
			Class<?> parceledClass;
			try {
				parceledClass = Class.forName(in.readString());
				FNObject model = (FNObject) parceledClass.newInstance();
		...
		@Override
		public FNObject[] newArray(int size) {
			return new FNObject[size];
		}
	};
	...
	protected static void storeFields(FNObject model, Parcel out, int flags) throws IllegalArgumentException, IllegalAccessException {
		Field[] fields = model.getClass().getFields();
		Arrays.sort(fields, compareMemberByName);

		for (Field field : fields) {
			if (Modifier.isStatic(field.getModifiers()) || field.isAnnotationPresent(FNTransient.class) ||
			...
			if (field.getType().equals(int.class) || field.getType().equals(Integer.class)) {
				...
				out.writeParcelable((FNObject) field.get(model), flags);
			} else {
	...
	protected static void restoreFields(FNObject model, Parcel in) throws IllegalArgumentException, IllegalAccessException {
		...
		for (Field field : fields) {
			...
			if (field.getType().equals(int.class) || field.getType().equals(Integer.class)) {
				...
				in.readTypedList((ArrayList) field.get(model), FNObject.CREATOR);
			}
			else if (List.class.isAssignableFrom(field.getType())) {
				in.readTypedList((List) field.get(model), FNObject.CREATOR);
			}
			else if (Hashtable.class.isAssignableFrom(field.getType())) {
				field.set(model, in.readSerializable());
			}else if (HashMap.class.isAssignableFrom(field.getType())) {
				field.set(model, in.readHashMap(FNObject.class.getClassLoader()));
			} else if (Map.class.isAssignableFrom(field.getType())) {
				field.set(model, in.readHashMap(FNObject.class.getClassLoader()));
			} else if (FNObject.class.isAssignableFrom(field.getType())) {
	...
	public boolean boolValueForKey(String key) {
		return FNObjectUtil.boolValue(this.valueForKey(key));
	}

	public long longValueForKey(String key) {
		return FNObjectUtil.longValue(this.valueForKey(key));
	}

	public int intValueForKey(String key) {
		return FNObjectUtil.intValue(this.valueForKey(key));
	}

	public double doubleValueForKey(String key) {
		return FNObjectUtil.floatValue(this.valueForKey(key));
	}
	...
	public boolean boolValueForSerializedName(String name) {
		return FNObjectUtil.boolValue(this.valueForSerializedName(name));
	}

	public Object valueForKeyPath(String aKeyPath) {
		return FNObjectUtil.valueForKeyPath(aKeyPath, this);
	}

	public Object valueForKey(String key) {
		return FNObjectUtil.valueForKey(key, this);
	}

	public Object valueForSerializedName(String name) {
		return FNObjectUtil.valueForSerializedName(name, this);
	}
	...
	public Map<String, Object> objectMapForKeys(String keys) {
		String[] keyArr = keys.split(FNSymbols.TILDE);
	...
	public boolean isEmpty(Object o) {
		return FNObjectUtil.isEmpty(o);
	}

	@Override
	public boolean isEmptyStr(String text) {
		return FNObjectUtil.isEmptyStr(text);
	}

	@Override
	public boolean isNonEmptyStr(String text) {
		return FNObjectUtil.isNonEmptyStr(text);
	}

	@Override
	public boolean isEmptyView(View v) {
		return FNObjectUtil.isEmptyView(v);
	}

	@Override
	public String getTextFromView(View v) {
		return FNObjectUtil.getTextFromView(v);
	}
