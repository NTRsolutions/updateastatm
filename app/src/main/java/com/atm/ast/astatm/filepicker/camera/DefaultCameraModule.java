...
import com.atm.ast.astatm.filepicker.model.PhotoFactory;
...
public class DefaultCameraModule implements FNCameraModule, Serializable {
	...
	public Intent getCameraIntent(Context context, FNFilePickerConfig config) {
		...
		File imageFile = FNImageUtil.createImageFile(config.getImageDirectory());
		if (imageFile != null) {
			...
			FNImageUtil.grantAppPermission(context, intent, uri);
			return intent;
		}
		return null;
	}
			...
			MediaScannerConnection.scanFile(context.getApplicationContext(),
					...
					new MediaScannerConnection.OnScanCompletedListener() {
						...
						public void onScanCompleted(String path, Uri uri) {
							...
							FNImageUtil.revokeAppPermission(context, imageUri);
						}
					});
		}
	}
