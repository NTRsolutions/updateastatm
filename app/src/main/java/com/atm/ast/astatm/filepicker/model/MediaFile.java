...
public class MediaFile extends AndroidDeviceFile {
	...
	public static MediaFile deviceFile(Cursor cursor, int position, int mediaType) {
		...
		int fileId = cursor.getInt(cursor.getColumnIndexOrThrow(FNFileUtil.idKey(mediaType)));
		String path = cursor.getString(cursor.getColumnIndexOrThrow(FNFileUtil.dataKey(mediaType)));
		String fileName = cursor.getString(cursor.getColumnIndexOrThrow(FNFileUtil.titleKey(mediaType)));

		MediaFile fileObj = new MediaFile(fileId, fileName, path, false);
		fileObj.setSize(cursor.getLong(cursor.getColumnIndexOrThrow(FNFileUtil.sizeKey(mediaType))));
		fileObj.setMimeType(cursor.getString(cursor.getColumnIndexOrThrow(FNFileUtil.mimeTypeKey(mediaType))));
	...
	public @DrawableRes int getResource() {
		switch (mediaType) {
			...
			default:
				return FNFileUtil.fileTypeImageResource(getMimeType());
		}
	}
