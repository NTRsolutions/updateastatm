package com.atm.ast.astatm.filepicker.view;
...
import com.atm.ast.astatm.filepicker.animation.SimpleValueAnimator;
import com.atm.ast.astatm.filepicker.animation.SimpleValueAnimatorListener;
import com.atm.ast.astatm.filepicker.animation.ValueAnimatorV14;
import com.atm.ast.astatm.filepicker.animation.ValueAnimatorV8;
...
public class FNCropImageView extends AppCompatImageView {
	...
	public FNCropImageView(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public FNCropImageView(Context context, AttributeSet attrs, int defStyle) {
	...
	private void setDefaults(float mDensity) {
		mBackgroundColor = ASTUIUtil.getColor(R.color.bg_color);
		mOverlayColor = ASTUIUtil.getColor(R.color.orange);
		mFrameColor = ASTUIUtil.getColor(R.color.colorPrimary);
		mHandleColor = ASTUIUtil.getColor(R.color.appTheamColor);
		mGuideColor = ASTUIUtil.getColor(R.color.appTheamColor);

		mHandleShowMode = ShowMode.SHOW_ALWAYS;
		mGuideShowMode = ShowMode.SHOW_ALWAYS;
		setGuideShowMode(mGuideShowMode);
		setHandleShowMode(mHandleShowMode);

		mHandleSize = (int) (HANDLE_SIZE_IN_DP * mDensity);
		mTouchPadding = (int) (8 * mDensity);
		...
		mIsCropEnabled = true;
		mInitialFrameScale = constrain(DEFAULT_INITIAL_FRAME_SCALE, 0.01f, 1.0f,
	...
	private void drawDebugInfo(Canvas canvas) {
		...
		builder.append("LOADED FROM: ")
				.append(sourceUrl != null ? "Url" : "Bitmap");
	...
	private Bitmap decodeRegion() {
		Bitmap cropped = null;
		try {
			BitmapRegionDecoder decoder = BitmapRegionDecoder.newInstance(sourceUrl, false);
		...
		//	FNLogUtil.sendToLogger("An error occurred while cropping the image: " + e.getMessage());
		} catch (OutOfMemoryError e) {
		//	FNLogUtil.sendToLogger("OOM Error: " + e.getMessage());
		} catch (Exception e) {
		//	FNLogUtil.sendToLogger("An unexpected error has occurred: " + e.getMessage());
		}
		return cropped;
	}
	...
	private Bitmap scaleBitmapIfNeeded(Bitmap cropped) {
		...
		if (outWidth > 0 && outHeight > 0) {
			Bitmap scaled = FNImageUtil.getScaledBitmap(cropped, outWidth, outHeight);
	...
	public void startCrop(final MediaFile file, CropCallback cropCallBack) {
		...
		mExecutor.submit(new Runnable() {
			...
			public void run() {
				...
				if (FNObjectUtil.isEmptyStr(sourceUrl)) {
				...
				file.setCropped(true);
				file.setPath(FNImageUtil.saveToFile(cropped, mCompressFormat, mCompressQuality));
				mHandler.post(new Runnable() {
					@Override
					public void run() {
						if (mCropCallback != null)
							mCropCallback.onSuccess(file);
					}
				});
				mIsCropping = false;
			}
