...
public class FNObjectUtil {
	...
	public static Object valueForKey(String aKey, Object anObject) {
		if (aKey != null) {
			if (anObject != null) {
				Class<?> aClass = classForObject(anObject);
				String aName = methodNameForKeyWithPrefix(aKey, FNObjectUtil.GetterPrefix);
	...
	public static Object valueForKeyPath(String aKeyPath, Object anObject) {
		...
		String[] path = aKeyPath.split(FNObjectUtil.KeyPathSeparatorRegEx);
		int len = path.length;
		Object current = anObject;

		for (int i = 0; i < len && current != null; i++) {
			if (current instanceof FNObject) {
